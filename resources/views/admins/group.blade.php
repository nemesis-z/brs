...
@section('content')
	...
	@forelse($tgls as $tgl)
		<div class="row" style="border-bottom:1px solid #eee;padding-bottom:10px;margin-bottom:10px;">
			<a class="col s5" href="/admin/lesson/{{$tgl->lesson->id}}">{{$tgl->lesson->name}} ({{$tgl->type_name()}}, {{$tgl->c_name()}})</a>
			...
			<a href="/teacher/get/list/{{$group->id}}/{{$tgl->lesson->id}}">Получить лист</a></div>
		</div>
	@empty
